...
func (a *addFeedScreen) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case nextFeedItem:
		a.idx = msg.idx
		a.processingStatus[a.idx] = processingStyle.Render(processingIcon + "\t" + a.feedsList[a.idx])
		a.progressBar.SetPercent(float64(a.idx) / float64(len(a.feedsList)))
...
func newScreen(ctx context.Context, a *adapter.FeedAdapter, feedsList []string) (*addFeedScreen, error) {
	...
	for _, feed := range feedsList {
		feedsProcessing = append(feedsProcessing, waitingStyle.Render(waitingIcon+"\t"+feed))
	}
...
func (a *addFeedScreen) Next() tea.Cmd {
	return func() tea.Msg {
		feedUrl := a.feedsList[a.idx]
		if feed, err := a.a.Parse(a.ctx, feedUrl); err != nil {
			a.processingStatus[a.idx] = failedStyle.Render(fmt.Sprintf("%s\t%s (%s)", failedIcon, feedUrl, err.Error()))
		} else {
...
func Start(ctx context.Context, a *adapter.FeedAdapter, feedsList []string) error {
	m, err := newScreen(ctx, a, feedsList)
	if err != nil {
